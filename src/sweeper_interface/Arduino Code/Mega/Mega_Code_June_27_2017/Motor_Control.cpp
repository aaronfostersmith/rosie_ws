...
void Motor_Control::update_pose(float pose[3])
{
  ...
  float d_right = (float)mCount_R * 1.122 * mSign_R; //2*pi*80mm/2
  float d_left = (float)mCount_L * 1.122 * mSign_L;
  ...
  //theta'd_baseline = 170 mm
  pose[2] += ((d_right - d_left) / 170) * mGainTheta;
...
void Motor_Control::update_PID()
{
  ...
  void Motor_Control::service_left_enc()
  {
    ...
    if (temp > 1000) {
      mCurSPD_L = 1122000 / (double) temp; // 1.122 [mm/tick]/ temp [s/tick] = speed in mm/s
